...
local function cmp(lhs, rhs)
    ...
    if tleft == "number" and tright == "number" then
        return lhs < rhs
    end
    if tleft == "string" and tright == "string" then
...
function pprint.pformat(obj, option, printer)
    ...
    local function table_formatter(t)
        ...
        for ix = 1, tlen do
            local v = t[ix]
            if formatter[type(v)] == nop_formatter or (option.filter_function and option.filter_function(v, ix, t)) then
                -- pass
            else
        ...
        local function print_kv(k, v, t)
            ...
            if
                ...
                or (option.filter_function and option.filter_function(v, k, t))
            then
                return
            end
            wrapped = _n()
